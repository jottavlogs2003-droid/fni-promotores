...
function RelatoriosPage() {
  ...
  function agrupar(rows: any[]) {
    ...
    rows.forEach(r => {
      ...
      const cur = map.get(chave) ?? { [tipo === "loja" ? "Loja" : tipo === "promotor" ? "Promotor" : "Cliente"]: chave, ...extra, Turnos: 0, Horas: 0, Diarias: 0, Custo: 0, Receita: 0 };
      cur.Turnos += 1;
    ...
    return Array.from(map.values()).map(x => ({ ...x, Lucro: x.Receita - x.Custo }));
  }
  ...
  return (
    <div className="space-y-5 animate-fade-in-up">
      <div>
        ...
        <p className="text-muted-foreground text-sm">Exporte em PDF ou Excel agrupado como preferir.</p>
      </div>
